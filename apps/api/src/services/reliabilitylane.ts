/**
 * ReliabilityLane — curated reliability patterns for AI agents (hosted API).
 * Match failure patterns, plan retries, run verification, resolve incidents
 * under /v1/reliabilitylane/*.
 */
...
export interface VerificationChecklist {
  id: string
  name: string
  area: 'deployment' | 'code' | 'security' | 'content' | 'data'
  checks: string[]
}
...
export function classifyError(input: { status?: number; code?: string; message?: string }): string {
...
export function matchFailure(input: { symptom?: string; error?: string; category?: string }) {
  const text = `${input.symptom || ''} ${input.error || ''}`.toLowerCase()
  const category = input.category?.toLowerCase()
  const scored: Array<{ pattern: FailurePattern; score: number; matchedTerms: string[] }> = []
...
export function planRetry(input: { status?: number; code?: string; message?: string; kind?: string }) {
...
export function verify(input: { checklist?: string; area?: string; evidence?: Record<string, unknown> }) {
  const checklist =
...
export function incidentFor(input: { symptom?: string; error?: string }) {
  const text = `${input.symptom || ''} ${input.error || ''}`.toLowerCase()
